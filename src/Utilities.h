...
bool isPointNearMonitorEdge(const sf::Vector2i& point, const sf::Vector2u& windowSize){
    ...
    //Imagine the position with an offset incase windows are not allowed to actually touch the edge of the screen like with KDE oder older windows version..
 
    if (point.x - 50 <= 0 || point.y - 50 <= 0) return true;

    if (point.x + 50 + windowSize.x >= screenSize.x) return true;

    if (point.y + 50 + windowSize.y >= screenSize.y) return true;
